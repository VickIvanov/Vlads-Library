...
function checkDatabaseConfig() {
  const requiredVars = ['POSTGRES_URL', 'POSTGRES_PRISMA_URL'];
  const missing = requiredVars.filter(v => !process.env[v]);
  
  if (missing.length > 0) {
    console.error('[DB] Отсутствуют переменные окружения:', missing);
    return false;
  }
  return true;
}
...
export async function addBook(bookData) {
  try {
    ...
    if (!checkDatabaseConfig()) {
      return { 
        success: false, 
        message: 'База данных не настроена. Создайте PostgreSQL базу данных в Vercel Storage (Storage → Create Database → Postgres).' 
      };
    }
