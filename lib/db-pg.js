...
export async function getUserPrivacySettings(username) {
  try {
    ...
    if (result.rows.length > 0) {
      ...
      return {
        show_favorites: row.show_favorites !== false, // true если true или null
        show_description: row.show_description !== false // true если true или null
      };
    }
...
export async function saveUserPrivacySettings(username, settings) {
  try {
    ...
    const showFavorites = settings.show_favorites === true || settings.show_favorites === 'true';
    const showDescription = settings.show_description === true || settings.show_description === 'true';
