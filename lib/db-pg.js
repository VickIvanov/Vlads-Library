...
function checkDatabaseConfig() {
  ...
  const possibleVars = [
    ...
    'POSTGRES_HOST', // Альтернативные варианты
    'POSTGRES_DATABASE'
  ];
  ...
  if (!found) {
    ...
    console.error('[DB] Доступные переменные окружения:', Object.keys(process.env).filter(k => k.includes('POSTGRES') || k.includes('DATABASE')).join(', ') || 'нет');
    return false;
  }
