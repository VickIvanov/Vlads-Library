...
export async function getUserPrivacySettings(username) {
  try {
    ...
    if (result.rows.length > 0) {
      ...
      // Возвращаем прямое значение из БД: если null, то true (по умолчанию), иначе фактическое значение
      return {
        show_favorites: row.show_favorites === null ? true : row.show_favorites === true,
        show_description: row.show_description === null ? true : row.show_description === true
      };
    }
...
export async function saveUserPrivacySettings(username, settings) {
  try {
    ...
    const sql = await getSql();
    // Явно преобразуем в boolean: если не true (включая false, null, undefined), то false
    const showFavorites = settings.show_favorites === true || settings.show_favorites === 'true' || settings.show_favorites === 1;
    const showDescription = settings.show_description === true || settings.show_description === 'true' || settings.show_description === 1;
    
    // Сохраняем как boolean в БД
    await sql`
      INSERT INTO user_privacy_settings (username, show_favorites, show_description, updated_at)
      VALUES (${username}, ${!!showFavorites}, ${!!showDescription}, CURRENT_TIMESTAMP)
