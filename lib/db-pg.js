...
export async function getUnreadCount(username) {
  try {
    ...
    const result = await sql`
      ...
      FROM messages
      WHERE receiver_username = ${username} AND read_status = FALSE
    `;
...
export async function getUserPrivacySettings(username) {
  try {
    ...
    if (result.rows.length > 0) {
      return {
        show_favorites: result.rows[0].show_favorites !== false,
        show_description: result.rows[0].show_description !== false,
        show_user_id: result.rows[0].show_user_id !== false
      };
    }
...
export async function saveUserPrivacySettings(username, settings) {
  try {
    ...
    const sql = await getSql();
    await sql`
      INSERT INTO user_privacy_settings (username, show_favorites, show_description, show_user_id, updated_at)
      VALUES (${username}, ${settings.show_favorites !== false}, ${settings.show_description !== false}, ${settings.show_user_id !== false}, CURRENT_TIMESTAMP)
      ON CONFLICT (username) DO UPDATE SET
        show_favorites = ${settings.show_favorites !== false},
        show_description = ${settings.show_description !== false},
        show_user_id = ${settings.show_user_id !== false},
        updated_at = CURRENT_TIMESTAMP
    `;
