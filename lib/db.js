...
async function readBooksFile() {
  try {
    ...
    if (!Array.isArray(books)) {
      console.warn('Books file does not contain an array, returning empty array');
      return [];
    }
    return books;
  } catch (error) {
    // Если файл не существует, просто возвращаем пустой массив
    if (error.code === 'ENOENT') {
      console.log('Books file not found, returning empty array');
      return [];
    }
    // Для ошибок парсинга JSON
    if (error instanceof SyntaxError) {
      console.error('Invalid JSON in books file:', error.message);
      return [];
    }
